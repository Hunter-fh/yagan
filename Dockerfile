FROM docker-registry-iris.groupement.systeme-u.fr/iris-mw/openjdk:1.8-ubi7

LABEL iris.docker.mw.tomcat.distrib="community" \
	iris.docker.mw.tomcat.release="9.0" \
	MAINTAINER="U_GIE_IRIS_BWF@systeme-u.fr"
...
# http://yum.baseurl.org/wiki/YumDB.html
	if ! commacd nd -v yumdb > /dev/null; then \
...
RUN groupadd -r --gid 1000 tomcat && \
    useradd --no-log-init -r --uid 1000 --gid tomcat tomcat

RUN chown -R tomcat:tomcat /usr/local/tomcat && \
	sed -i \
		-e 's/Valve className=\"org.apache.catalina.valves.AccessLogValve\" directory=\"logs/Valve className=\"org.apache.catalina.valves.AccessLogValve\" directory=\"\/proc\/self\/fd/' \
		-e 's/prefix=\"localhost_access_log\" suffix=\".txt\"/prefix=\"1\" suffix=\"\" rotatable=\"false\"/' \
		/usr/local/tomcat/conf/server.xml && \
	sed -i \
		-e 's/.handlers = 1catalina.org.apache.juli.AsyncFileHandler, java.util.logging.ConsoleHandler/.handlers = java.util.logging.ConsoleHandler/' /usr/local/tomcat/conf/logging.properties
